...
test("Cadastro de tabela", async function(){
    ...
    const response = await request('http://localhost:3000/tables','post',data);
    const newTable = response.data;
    const responseGetTable = await request(`http://localhost:3000/tables/${newTable.id}`,'get');
    ...
    await request(`http://localhost:3000/tables/${newTable.id}`,'delete');
});

test("Alteração de tabela", async function(){
    const responseNewTable = await request('http://localhost:3000/tables','post',dataTable());
    const newTable = responseNewTable.data;

    const newData = dataTable();
    const responseAlterTable = await request(`http://localhost:3000/tables/${newTable.id}`,'put', newData);
...
test("Pegar apenas uma tabela", async function(){
    const responseNewTable = await request('http://localhost:3000/tables','post',dataTable());
    const newTable = responseNewTable.data;
    const responseGetTables = await request('http://localhost:3000/tables','get');
